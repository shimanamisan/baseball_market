...
debug('===============================');
debug('=== 商品出品登録ページ registProduct.php ===');
debug('===============================');
debugLogStart();
...
// パラメータ改ざんチェック
//================================
// GETパラメータはあるが、改ざんされている（URLをいじった）場合、正しい商品データが取れないのでマイページへ遷移させる
  ...
  if(empty($err_msg)){
    ...
    try {
      ...
      if($edit_flg){
        ...
        debug('DB新規登録です。');
        $sql = 'INSERT INTO product (name, maker_id, category_id, price, comment, pic1, pic2, pic3, user_id, create_date )
                VALUES (:name, :maker, :category, :price, :comment,  :pic1, :pic2, :pic3, :u_id, :date)';
        $data = array(
        ':name' => $name ,
        ':maker' => $maker,
        ':category' => $category,
      ...
      <section id="main" >
        <div class="form-container">
          <form action="" method="post" class="form" enctype="multipart/form-data" style="width:100%;box-sizing:border-box;">
            ...
            <label class="<?php if(!empty($err_msg['maker_id'])) echo 'err'; ?>">
              メーカー<span class="label-require">必須</span>
              <select name="maker_id">
                <option value="0" <?php if(getFormData('maker_id') == 0){echo 'selected'; } ?>>選択してください</option>
                  <?php
                    foreach($dbMakerData as $key => $val){
                  ?>
                  <option value="<?php echo $val['id'] ?>" <?php if(getFormData('maker_id') == $val['id']){ echo 'selected' ;} ?>>
                      <?php echo $val['name']; ?>
                  </option>
                  <?php
                    }
            ...
            <label class="<?php if(!empty($err_msg['comment'])) echo 'err'; ?>">
              詳細<span id="js-textCount-Error" class="textCount-Error"></span>
            ...
            <div style="overflow:hidden;">
              <div class="imgDrop-container">
                ...
                <!-- 最大サイズを決める場合には input type="hidden" で必ず設置する -->
