...
$(function () {
  ...
  var $jsShowMsg = $("#js-show-msg"); // HTML出力されているメッセージの中身を取得

  var msg = $jsShowMsg.text(); // HTML側で微妙に空白があるので、取り除いて文字が存在していると判定する
  ...
  var $fileInput = $(".input-file"); // cssプロパティ用のオブジェクト
  ...
  $dropArea.on("dragover", function (e) {
    // 親要素にイベントが伝播するのを防ぐ
    e.stopPropagation(); // クリックイベントなどをキャンセルさせる
  ...
  $fileInput.on("change", function (e) {
    ...
    $dropArea.css(dragleaveObject); // 2.files配列にファイルが入っています、this($fileInput)はinputタグにつけられたクラスでファイル情報を取ってきている

    var file = this.files[0],
        // 3 .上記のthisに対して、$(this)としてjQueryオブジェクトにしている、そうすることでjQueryのsiblingsメソッドで兄弟（同列）のimgを取得
    $img = $(this).siblings(".prev-img"),
        // 4.ファイルを読み込むFileReaderオブジェクト
    fileReader = new FileReader(); // console.log($img);
    ...
    fileReader.readAsDataURL(file); // console.log(fileReader);
  }); // トップへ戻るボタン
  ...
  $switchImgSubs.on("click", function (e) {
    ...
    $switchImgMain.attr("src", $(this).attr("src")); //attr属性の値を取得してその値を返す
  }); // お気に入り追加・削除

  var $like, likeProductId; // もしDOMが無かった場合にundefinedではなく、nullが入って後続の処理が止まらにようにする

  $like = $(".js-click-like") || null;
  likeProductId = $like.data("productid") || null; // 数値の0はfalseと判定されてしまう
  ...
  *****************************************/
  ...
  $('a[href^="#"]').on("click", function () {
    // クリックした要素のhref属性を取得
    var href = $(this).attr("href"); // 条件：上記で取得したhref属性が # かつ 空文字 であれば "html" と言う文字列を返す。そうでなければ取得してきたhtml属性を返す
    // したがって、条件がtrueだったら "html" が返ってくるので $("html") と言うエレメントを入れていることになる
    // 条件がfalseであれば $(this).attr("href") で取得したエレメントが入ってくる

    var target = $(href == "#" || href === "" ? "html" : href); // documentを起点として要素の座標を取得
...
var $inputEmail = $(".js-guest-email");
var $inputPass = $(".js-guest-password");
var $guestLogin = $(".js-guest-login");
