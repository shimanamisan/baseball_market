    ...
    if(empty($err_msg)){
      ...
      try{
        ...
        if($stmt && array_shift($result)){
          debug('クエリ成功！！ DB登録あり！！ passRemaindSend.php');
...
パスワード再発行認証キー入力ページ：http://localhost/webukatu/Lesson17/passRemindRecieve.php
認証キー：{$auth_key}
※認証キーの有効期限は30分となります

認証キーを再発行されたい場合は下記ページより再度お手続きをお願い致します。
http://localhost/webukatu/Lesson17/passRemaindSend.php
...
EOT;
          ...
          debug('ここの処理は見えていますか？ *** heaserメソッドの後 -> passRemaindSend.php ***');
